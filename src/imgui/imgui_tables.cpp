...
static void TableSetupColumnFlags( ImGuiTable* table, ImGuiTableColumn* column, ImGuiTableColumnFlags flags_in )
{
    ...
    if ( table->Flags & ImGuiTableFlags_Sortable )
    {
        ...
        if ( ( flags & ImGuiTableColumnFlags_PreferSortAscending ) != 0 && ( flags & ImGuiTableColumnFlags_NoSortAscending ) == 0 ) {
            mask |= 1 << ImGuiSortDirection_Ascending;  list |= ImGuiSortDirection_Ascending << ( count << 1 ); count++;
        }
        if ( ( flags & ImGuiTableColumnFlags_PreferSortDescending ) != 0 && ( flags & ImGuiTableColumnFlags_NoSortDescending ) == 0 ) {
            mask |= 1 << ImGuiSortDirection_Descending; list |= ImGuiSortDirection_Descending << ( count << 1 ); count++;
        }
        if ( ( flags & ImGuiTableColumnFlags_PreferSortAscending ) == 0 && ( flags & ImGuiTableColumnFlags_NoSortAscending ) == 0 ) {
            mask |= 1 << ImGuiSortDirection_Ascending;  list |= ImGuiSortDirection_Ascending << ( count << 1 ); count++;
        }
...
void ImGui::TableBeginCell( ImGuiTable* table, int column_n )
{
    ...
    {
        LogRenderedText( &window->DC.CursorPos, _( "|" ) );
...
void ImGui::TableHeader( const char* label )
{
    ...
    if ( ( table->Flags & ImGuiTableFlags_Sortable ) && !( column->Flags & ImGuiTableColumnFlags_NoSort ) )
    {
        ...
        {
            ImFormatString( sort_order_suf, IM_ARRAYSIZE( sort_order_suf ), _( "%d" ), column->SortOrder + 1 );
    ...
    if ( text_clipped && hovered && g.ActiveId == 0 )
        SetItemTooltip( _( "%.*s" ), ( int )( label_end - label ), label );
...
void ImGui::TableAngledHeadersRowEx( float angle, float max_label_width )
{
    ...
    const ImGuiID row_id = GetID( _( "##AngledHeaders" ) );
...
void ImGui::TableOpenContextMenu( int column_n )
{
    ...
    if ( table->Flags & ( ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable ) )
    {
        ...
        table->InstanceInteracted = table->InstanceCurrent;
        const ImGuiID context_menu_id = ImHashStr( _( "##ContextMenu" ), 0, table->ID );
...
static void* TableSettingsHandler_ReadOpen( ImGuiContext*, ImGuiSettingsHandler*, const char* name )
{
    ImGuiID id = 0;
    int columns_count = 0;
    if ( sscanf( name, _( "0x%08X,%d" ), &id, &columns_count ) < 2 )
        return NULL;
...
static void TableSettingsHandler_ReadLine( ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line )
{
    ...
    if ( sscanf( line, _( "RefScale=%f" ), &f ) == 1 ) {
        settings->RefScale = f; return;
    }

    if ( sscanf( line, _( "Column %d%n" ), &column_n, &r ) == 1 )
    {
        ...
        if ( sscanf( line, _( "UserID=0x%08X%n" ), ( ImU32* )&n, &r ) == 1 ) {
            line = ImStrSkipBlank( line + r ); column->UserID = ( ImGuiID )n;
        }
        if ( sscanf( line, _( "Width=%d%n" ), &n, &r ) == 1 ) {
            line = ImStrSkipBlank( line + r ); column->WidthOrWeight = ( float )n; column->IsStretch = 0; settings->SaveFlags |= ImGuiTableFlags_Resizable;
        }
        if ( sscanf( line, _( "Weight=%f%n" ), &f, &r ) == 1 ) {
            line = ImStrSkipBlank( line + r ); column->WidthOrWeight = f; column->IsStretch = 1; settings->SaveFlags |= ImGuiTableFlags_Resizable;
        }
        if ( sscanf( line, _( "Visible=%d%n" ), &n, &r ) == 1 ) {
            line = ImStrSkipBlank( line + r ); column->IsEnabled = ( ImU8 )n; settings->SaveFlags |= ImGuiTableFlags_Hideable;
        }
        if ( sscanf( line, _( "Order=%d%n" ), &n, &r ) == 1 ) {
            line = ImStrSkipBlank( line + r ); column->DisplayOrder = ( ImGuiTableColumnIdx )n; settings->SaveFlags |= ImGuiTableFlags_Reorderable;
        }
        if ( sscanf( line, _( "Sort=%d%c%n" ), &n, &c, &r ) == 2 ) {
            line = ImStrSkipBlank( line + r ); column->SortOrder = ( ImGuiTableColumnIdx )n; column->SortDirection = ( c == '^' ) ? ImGuiSortDirection_Descending : ImGuiSortDirection_Ascending; settings->SaveFlags |= ImGuiTableFlags_Sortable;
        }
    }
}
...
static void TableSettingsHandler_WriteAll( ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf )
{
    ...
    for ( ImGuiTableSettings* settings = g.SettingsTables.begin( ); settings != NULL; settings = g.SettingsTables.next_chunk( settings ) )
    {
        ...
        for ( int column_n = 0; column_n < settings->ColumnsCount; column_n++, column++ )
        {
            ...
            if ( save_size && !column->IsStretch ) {
                buf->appendf( _( " Width=%d" ), ( int )column->WidthOrWeight );
            }
            if ( save_visible ) {
                buf->appendf( _( " Visible=%d" ), column->IsEnabled );
            }
            if ( save_order ) {
                buf->appendf( _( " Order=%d" ), column->DisplayOrder );
            }
            if ( save_sort && column->SortOrder != -1 ) {
                buf->appendf( _( " Sort=%d%c" ), column->SortOrder, ( column->SortDirection == ImGuiSortDirection_Ascending ) ? 'v' : '^' );
            }
            buf->append( _( "\n" ) );
        }
        buf->append( _( "\n" ) );
...
void ImGui::TableSettingsAddSettingsHandler( )
{
    ...
    ini_handler.TypeName = _( "Table" );
    ini_handler.TypeHash = ImHashStr( _( "Table" ) );
...
void ImGui::DebugNodeTable( ImGuiTable* table )
{
    ...
    bool open = TreeNode( table, _( "Table 0x%08X (%d columns, in '%s')%s" ), table->ID, table->ColumnsCount, table->OuterWindow->Name, is_active ? _( "" ) : _( " *Inactive*" ) );
    if ( !is_active ) {
        PopStyleColor( );
    }
    if ( IsItemHovered( ) )
    ...
    if ( !open )
        return;
    if ( table->InstanceCurrent > 0 )
        Text( _( "** %d instances of same table! Some data below will refer to last instance." ), table->InstanceCurrent + 1 );
    bool clear_settings = SmallButton( _( "Clear settings" ) );
    BulletText( _( "OuterRect: Pos: (%.1f,%.1f) Size: (%.1f,%.1f) Sizing: '%s'" ), table->OuterRect.Min.x, table->OuterRect.Min.y, table->OuterRect.GetWidth( ), table->OuterRect.GetHeight( ), DebugNodeTableGetSizingPolicyDesc( table->Flags ) );
    BulletText( _( "ColumnsGivenWidth: %.1f, ColumnsAutoFitWidth: %.1f, InnerWidth: %.1f%s" ), table->ColumnsGivenWidth, table->ColumnsAutoFitWidth, table->InnerWidth, table->InnerWidth == 0.0f ? " (auto)" : "" );
    BulletText( _( "CellPaddingX: %.1f, CellSpacingX: %.1f/%.1f, OuterPaddingX: %.1f" ), table->CellPaddingX, table->CellSpacingX1, table->CellSpacingX2, table->OuterPaddingX );
    BulletText( _( "HoveredColumnBody: %d, HoveredColumnBorder: %d" ), table->HoveredColumnBody, table->HoveredColumnBorder );
    BulletText( _( "ResizedColumn: %d, ReorderColumn: %d, HeldHeaderColumn: %d" ), table->ResizedColumn, table->ReorderColumn, table->HeldHeaderColumn );
    for ( int n = 0; n < table->InstanceCurrent + 1; n++ )
    {
        ImGuiTableInstanceData* table_instance = TableGetInstanceData( table, n );
        BulletText( _( "Instance %d: HoveredRow: %d, LastOuterHeight: %.2f" ), n, table_instance->HoveredRowLast, table_instance->LastOuterHeight );
    }
    ...
    for ( int n = 0; n < table->ColumnsCount; n++ )
    {
        ...
        ImFormatString( buf, IM_ARRAYSIZE( buf ),
                        _( "Column %d order %d '%s': offset %+.2f to %+.2f%s\n"
                        "Enabled: %d, VisibleX/Y: %d/%d, RequestOutput: %d, SkipItems: %d, DrawChannels: %d,%d\n"
                        "WidthGiven: %.1f, Request/Auto: %.1f/%.1f, StretchWeight: %.3f (%.1f%%)\n"
                        "MinX: %.1f, MaxX: %.1f (%+.1f), ClipRect: %.1f to %.1f (+%.1f)\n"
                        "ContentWidth: %.1f,%.1f, HeadersUsed/Ideal %.1f/%.1f\n"
                        "Sort: %d%s, UserID: 0x%08X, Flags: 0x%04X: %s%s%s.." ),
                        n, column->DisplayOrder, name, column->MinX - table->WorkRect.Min.x, column->MaxX - table->WorkRect.Min.x, ( n < table->FreezeColumnsRequest ) ? _( " (Frozen)" ) : _( "" ),
                        column->IsEnabled, column->IsVisibleX, column->IsVisibleY, column->IsRequestOutput, column->IsSkipItems, column->DrawChannelFrozen, column->DrawChannelUnfrozen,
                        column->WidthGiven, column->WidthRequest, column->WidthAuto, column->StretchWeight, column->StretchWeight > 0.0f ? ( column->StretchWeight / sum_weights ) * 100.0f : 0.0f,
                        column->MinX, column->MaxX, column->MaxX - column->MinX, column->ClipRect.Min.x, column->ClipRect.Max.x, column->ClipRect.Max.x - column->ClipRect.Min.x,
                        column->ContentMaxXFrozen - column->WorkMinX, column->ContentMaxXUnfrozen - column->WorkMinX, column->ContentMaxXHeadersUsed - column->WorkMinX, column->ContentMaxXHeadersIdeal - column->WorkMinX,
                        column->SortOrder, ( column->SortDirection == ImGuiSortDirection_Ascending ) ? _( " (Asc)" ) : ( column->SortDirection == ImGuiSortDirection_Descending ) ? _( " (Des)" ) : _( "" ), column->UserID, column->Flags,
                        ( column->Flags & ImGuiTableColumnFlags_WidthStretch ) ? _( "WidthStretch " ) : "",
                        ( column->Flags & ImGuiTableColumnFlags_WidthFixed ) ? _( "WidthFixed " ) : "",
                        ( column->Flags & ImGuiTableColumnFlags_NoResize ) ? _( "NoResize " ) : "" );
        Bullet( );
        Selectable( buf );
        if ( IsItemHovered( ) )
        {
...
void ImGui::DebugNodeTableSettings( ImGuiTableSettings* settings )
{
    if ( !TreeNode( ( void* )( intptr_t )settings->ID, "Settings 0x%08X (%d columns)", settings->ID, settings->ColumnsCount ) )
        return;
    BulletText( _( "SaveFlags: 0x%08X" ), settings->SaveFlags );
    BulletText( _( "ColumnsCount: %d (max %d)" ), settings->ColumnsCount, settings->ColumnsCountMax );
    for ( int n = 0; n < settings->ColumnsCount; n++ )
    {
        ImGuiTableColumnSettings* column_settings = &settings->GetColumnSettings( )[ n ];
        ImGuiSortDirection sort_dir = ( column_settings->SortOrder != -1 ) ? ( ImGuiSortDirection )column_settings->SortDirection : ImGuiSortDirection_None;
        BulletText( _( "Column %d Order %d SortOrder %d %s Vis %d %s %7.3f UserID 0x%08X" ),
                    n, column_settings->DisplayOrder, column_settings->SortOrder,
                    ( sort_dir == ImGuiSortDirection_Ascending ) ? _( "Asc" ) : ( sort_dir == ImGuiSortDirection_Descending ) ? _( "Des" ) : _( "---" ),
                    column_settings->IsEnabled, column_settings->IsStretch ? _( "Weight" ) : _( "Width " ), column_settings->WidthOrWeight, column_settings->UserID );
    }
    TreePop( );
}
...
ImGuiID ImGui::GetColumnsID( const char* str_id, int columns_count )
{
    ...
    ImGuiID id = window->GetID( str_id ? str_id : _( "columns" ) );
    PopID( );
